...
func (c *Client) Chat(ctx context.Context, req llm.Request) (*llm.Response, error) {
	...
	neutralMsg := neutralMessage(choice.Message)
	resp := &llm.Response{
		ID:           envelope.ID,
		Message:      neutralMsg,
...
func (c *Client) post(ctx context.Context, body wireRequest) (*http.Response, string, time.Time, error) {
	...
	if err != nil {
		...
		if !errors.Is(err, io.EOF) {
			return nil, "", time.Time{}, fmt.Errorf("chat completions: %w", err)
		}
		body, getErr := httpReq.GetBody()
		if getErr != nil {
			return nil, "", time.Time{}, fmt.Errorf("chat completions: retry unavailable: %w", err)
		}
		httpReq.Body = body
		httpResp, err = c.httpClient().Do(httpReq)
		if err != nil {
