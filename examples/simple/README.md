...
- `reviewer` - a judge: after each of simple's runs finishes, the reviewer receives the run's transcript and checks its claims against the knowledgebase with its own `read` and `search` tools, then reports a verdict. The judgement prints after the run's output; nothing else consumes it.
